...
use \XTPSolutions\Http\Request,
    \XTPSolutions\Http\Response,
    \XTPSolutions\Http\RequestParams,
    \XTPSolutions\Exceptions\InvalidRequest,
    \XTPSolutions\Utilities\Config as Config;
...
class BaseResource {
  ...
  public function __construct($params = array()) {
    $this->requestOptions = new \XTPSolutions\Http\RequestParams();
    if ($params) {
  ...
  public function update($params = array()) {
    ...
    if (!isset($values["id"])) {
      if (Config::exceptionsEnabled()) {
        throw new \XTPSolutions\Exceptions\InvalidRequest("ID is required for this action");
      }
      else {
        return false;
      }
  ...
  public function setOptions($params = false) {
    if ($params && get_class($params) === "XTPSolutions\Http\RequestParams") {
          ...
          if (is_array($value)) {
            ...
            foreach ($value as $subKey => $subVal) {
              if ($subVal instanceof BaseResource) {
                ...
                throw new \XTPSolutions\Exceptions\InvalidRequest("Incorrect nesting structure for request");
              }
            }
          }
  ...
  private function _validateResponse() {
    ...
    if ($this->response->hasErrors()) {
      if (Config::exceptionsEnabled()) {
        throw new \XTPSolutions\Exceptions\ApiErrors('There are errors in the response');
      }
      return false;
    }
    return true;
